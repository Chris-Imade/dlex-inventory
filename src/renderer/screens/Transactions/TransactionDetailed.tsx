...
const TransactionDetailed = () => {
  ...
  return (
    <div
      className={`py-[48px] px-[49px] w-[93.8vw] h-[100vh] bg-white ${styles.dashboard}`}
    >
      <h4 style={{ fontFamily: fonts.family.medium }} className="text-2xl">
        Transaction Details
      </h4>
      <div className="mt-6">
        ...
        <div
          className={`w-full overflow-x-auto border-[1px] border-solid border-slate-500 h-[20rem] flex justify-between items-center ${styles.dashboard}`}
        >
          {data?.products.map((product: TransDetailProd, index: number) => {
            ...
            return (
              <div
                key={index}
                className={`flex items-center min-w-[40rem] hover:cursor-pointer bg-gray-200 p-3 rounded-lg mr-3 ${
                  index !== 0 && index !== data.products.length - 1
                    ? 'mx-2'
                    : ''
                }`}
              >
          ...
          <table className="w-full border-collapse border border-gray-400">
            ...
            <tbody>
              <tr className={'bg-white'}>
                <td
                  style={{ fontFamily: fonts.family.medium }}
                  className="uppercase border border-gray-400 px-4 py-2"
                >
                  Customer's Name
                </td>
                <td
              ...
              <tr className={`${data?.phoneTwo ? 'bg-white' : 'bg-gray-100'}`}>
                <td
                  style={{ fontFamily: fonts.family.medium }}
                  className="uppercase border border-gray-400 px-4 py-2 text-red-500"
                >
                  {'Project Deadline'}
                </td>
                <td
                ...
                >
                  {data.deadline
                    ? new Date(data.deadline as any).toLocaleDateString()
                    : ''}
                </td>
              </tr>
            </tbody>
