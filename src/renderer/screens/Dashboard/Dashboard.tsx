...
import { setActiveRoute, setCompletedTasks, setTotalSales, setWorkInProgress } from 'renderer/Redux/Splice/appSlice';

const Dashboard = () => {

  const totalSales = useSelector((state: AppState) => state.data.totalSales);
  const workInProgress = useSelector((state: AppState) => state.data.workInProgress);
  const completedDeals = useSelector((state: AppState) => state.data.completedDeals);
  ...
  const featuredProducts = products && products.length > 0 ? products.slice(-4) : [];
  ...
  useEffect(() => {
    const currentProjects = actualTransactions?.filter((item: any) => item.status === 'Pending');
    const closedProjects = actualTransactions?.filter((item: any) => item.status === 'Closed');
  ...
  return (
    ...
    >
      ...
      <div className={'mt-[29px] flex lg:flex-row min-w-[20rem] overflow-x-auto featured'}>
        <div className="mr-4">
          ...
          featuredProducts?.length !== 0 && (
            <>
              ...
              <div className="flex md:flex-row min-w-[20rem] overflow-x-auto featured">
                {featuredProducts && featuredProducts.map((item, _) => {
