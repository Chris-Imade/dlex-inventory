...
const Transaction: React.FC<TransactionListProps> = ({ item }) => {
  ...
  const handleSaveStatus = async(status: string, id: string | undefined) => {
    ...
    if(navigator.onLine) {
      ...
      try {
        const response = await fetch(`${baseURL}${`/api/v1/transaction/${id}`}`, {
            method: "PUT",
            headers: {
      ...
      <>
        ...
        >
          ...
          <div className="flex justify-between items-center flex-grow ml-12 md:ml-36">
              ...
              {showStatusModal && (
                <div className={`animated-modal p-2 pt-4 w-44`}>
                  ...
                  <div className='flex justify-between items-center'>
                    <button className='px-2 py-1 text-white rounded-md' onClick={() => setShowStatusModal(false)}>
                      <img src={images.close} className='' width={20} alt="close" />
                    </button>
                    <button className='px-2 py-1 text-white rounded-md' onClick={() => {
                      // You can access the selected status here with the state variable selectedStatus
                      setShowStatusModal(false);
                      console.log('Selected Status:', selectedStatus, "Item ID:", item._id);
            ...
            <CustomDropdown item={item} handleDropdown={handleDropdown} showDropDown={showDropDown} />
          </div>
        </div>
        {showStatus && (
